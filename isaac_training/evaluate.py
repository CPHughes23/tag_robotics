...
parser.add_argument(
    "--scenario",
    type=str,
    choices=["small_runner", "large_runner"],
    default="small_runner",
    help="Which physical car plays runner: 'small_runner' or 'large_runner'",
)
parser.add_argument("--runner_checkpoint", type=str, required=True,
                    help="Path to the runner policy .pt file")
parser.add_argument("--chaser_checkpoint", type=str, required=True,
                    help="Path to the chaser policy .pt file")
...
from envs.rc_car_env import (
    RCCarEnv,
    RCCarEvalEnvCfg,
    RCCarEvalEnvCfgLargeRunner,
)
...
def main():
    if args_cli.scenario == "small_runner":
        env_cfg = RCCarEvalEnvCfg()
    else:
        env_cfg = RCCarEvalEnvCfgLargeRunner()
    ...
    with torch.inference_mode():
        while simulation_app.is_running():
            ...
            obs, _, _, _, _ = env.step({"runner": runner_actions, "chaser": chaser_actions})
